...
### APPENDICES S9-S11: Polynomial Relationships within each site ###

# APPENDIX S9 - Richness ~ Aboveground Biomass #
polyS9<-ggplot(forbgram,aes(x=plotanpp,y=gramforbrich,color=Site))+
  geom_point(position=position_jitter(width=0.05, height=0.05),
        alpha=0.2, color="black")+geom_smooth(method=lm,formula=y~poly(x,2),
...
# APPENDIX S10 - Richness ~ Relative Wiregrass Cover #
polyS10<-ggplot(forbgram,aes(x=wirerelcov,y=gramforbrich,color=Site))+
  geom_point(position=position_jitter(width=0.05, height=0.05),
    alpha=0.2, color="black")+geom_smooth(method=lm,formula=y~poly(x,2),
    se=FALSE,method.args = list(family=gaussian),fill=NA, lwd=1.5,alpha=0.1,
    linetype="dashed")+theme(strip.text = element_text(size = 12),
...
# APPENDIX S11 - Aboveground Biomass ~ Relative Wiregrass Cover #
polyS11<-ggplot(forbgram,aes(x=wirerelcov,y=plotanpp,color=Site))+
  geom_point(position=position_jitter(width=0.05, height=0.05),
    alpha=0.2, color="black")+geom_smooth(method=lm,formula=y~poly(x,2),
    se=FALSE,method.args = list(family=gaussian),fill=NA, lwd=1.5,alpha=0.1,
    linetype="dashed")+theme(strip.text = element_text(size = 12),
